...
def run_and_plot_pca(representations_path, save_filename):
    ...
    for path in representations_path:
        ...
        representations.append(df.T.as_matrix()[0])
    # Do the PCA thing:
    pca = PCA()
...
for stimuli_dir, reps_dir, figs_dir, stimuli_postfix, title \
    ...
    correlation_df['Frequency'] = correlation_df.apply(max_correlation,
                                                       column_name='Frequency', axis=1)
    # Save the max correlation involving a PC and Orientation
    correlation_df['Orientation'] = correlation_df.apply(max_correlation,
    ...
    for l, layer_name in enumerate(LAYER_NAMES):
        ...
        if not os.path.isfile(permutations_filename):
            for permutation in range(PERMUTATIONS):
                ...
                try:
                    ...
                    permutation_df = pd.DataFrame(columns, index=[permutation])
    ...
    for column in ['Frequency', 'Orientation']:
        ...
        for l, layer_name in enumerate(LAYER_NAMES):
            ...
            permutation_df = permutation_df.sort_values(by=[permuted_column])
            ...
            # print('permuted cut-off correlation', permutation_correlation_cut_off)
            permuted_p_value_less_than_alpha.append(high_permutation_correlation_cut_off < correlation)
        correlation_df[new_column] = permuted_p_value_less_than_alpha

        # print(permutation_correlation_cut_off < correlation)
        # print(len(permuted_p_value_less_than_alpha))
        # print(len(correlation_df))
    print(correlation_df.columns, correlations_results_filename)
    correlation_df['Layer'] = correlation_df.index
