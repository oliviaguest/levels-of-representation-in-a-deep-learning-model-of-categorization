...
if __name__ == '__main__':
    ...
    accuracy_colour = df['Optimum Accuracy']
    df = pd.read_csv(ACCURACY_DIR +
                     'circle_square_bounding_box_grayscale_accuracy.csv')
    accuracy_grayscale = df['Optimum Accuracy']
    filename = 'optimum_choice_bounding_box_accuracy.pdf'
    create_and_save_figure(
        accuracy_colour, accuracy_grayscale, filename, y_min=0.49, y_max=1.005)
    ...
    accuracy_grayscale = df['Optimum Accuracy']
    filename = 'optimum_choice_accuracy.pdf'
    create_and_save_figure(
        accuracy_colour, accuracy_grayscale, filename, y_min=0.49, y_max=1.005)
    ...
    df = pd.read_csv(ACCURACY_DIR +
                     'circle_square_bounding_box_colour_accuracy.csv')
    accuracy_shape = df['Same Shape Optimum Accuracy']
    accuracy_hue = df['Same Hue Optimum Accuracy']
    accuracy_size = df['Same Size Optimum Accuracy']
    filename = 'optimum_shape_hue_size_accuracy.pdf'
    y_min = 0.49
    y_max = 1.005
